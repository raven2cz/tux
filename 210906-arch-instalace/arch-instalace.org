...
** Network konfigurace
rcman eflector -c "CZ" -f 12 -l 10 -n 12 --save /etc/pacman.d/mirrorlist
Zadejte svůj vymyšlený název počítače do ~/etc/hostname~. V mém případě =r7arch=. Pozor nesmí obsahovat pomlčky.
